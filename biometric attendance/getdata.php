            ...
            if (!empty($row['username'])){
                ...
                if (!mysqli_stmt_prepare($result, $sql)) {
                    ...
                    //Logout
                    	...
                    	$sql="UPDATE users_logs u, (SELECT MAX(id) as lastid FROM users_logs AS u1 WHERE u1.fingerprint_id=? AND u1.checkindate=CURDATE())  u1
							SET u.timeout=CURTIME() WHERE u.fingerprint_id=? AND u.checkindate=CURDATE()
							AND u.id= u1.lastid";
                        $result = mysqli_stmt_init($conn);
                        ...
                        if (!mysqli_stmt_prepare($result, $sql)) {
                            ...
                            mysqli_stmt_bind_param($result, "ii", $fingerID,$fingerID);
